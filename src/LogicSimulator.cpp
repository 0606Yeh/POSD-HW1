...
void LogicSimulator::setGate(int index, string text){
    ...
    while(information != 0){
        ...
        if(information < 0){
            circuit[index]->addInputPin(iPins[information - 1]);
        }else if(information > 0){
            circuit[index]->addInputPin(circuit[information - 1]);
...
bool LogicSimulator::load(string path){
    ...
    for(int i = 0; i < gate_is_connect.size(); i++){
        if(!gate_is_connect[i])
            oPins[i]->addInputPin(circuit[i]);
        else{
            delete oPins[i];
            oPins.erase(oPins.begin() + i);
            i--;
        }
    } 
}
